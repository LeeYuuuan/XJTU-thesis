...
\subsubsection{图像重建块实现}{Implementation of Image Reconstruction Block}
...
然而，由于原始DDPM中的去噪网络的输入仅为隐变量$x_t$以及时刻$t$，EIT问题中电导率分布的生成却需要施加一定的边界条件（即电压-电流信号），
...
本节将首先介绍如何利用Cross-Attention机制将电压分布嵌入到网络的输入中，然后再阐述本文所实现的生成式EIT图像重建算法中生成噪声的网络的架构以及整个图像重建块的架构。
...
\subsubsection{生成噪声网络的架构}{Architecture of Noise Generation Network}

利用上述CrossAttentionLayer，即可高效地将电压数据嵌入生成噪声的网络中，该网络同样采用U-net作为基本架构，如\cref{figure:Unet}所示。
...
其中，输入的时刻信息$t$为 原始时刻$t_{ori}, t \in \mathbb{R}^1$ 经过TimeEmbedding层线性变换后的代表时刻信息的向量 $t, t\in \mathbb{R}^1024$。

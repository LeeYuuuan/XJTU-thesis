...
\xsection{用于脑卒中快速鉴别的生成式EIT图像重建算法设计}{Design of Generative EIT Reconstruction Algorithm for Fast Detection of Stroke}
...
\xsubsection{用于脑卒中快速鉴别的生成式EIT图像重建算法总述}{Overall of of Generative EIT Reconstruction Algorithm for Fast Detection of Stroke}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.8\textwidth]{EITREmodel.png}
...
\begin{table}[H]
    ...
    层数 & 输入 & 网络类型  & 输出\\
    \midrule
    1 & $input$ & Linear & $v_1=\text{Linear}(input)$, $residue_1=input$  \\
...
VEncoder中的第2层为归一化层，用于对上一层的输出归一化，采用层归一化策略（Layer Normalization, LN）。
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.8\textwidth]{ReconsBlock.jpg}
    \caption{图像重建块}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.8\textwidth]{DP.jpg}
    \caption{正向过程}
    \label{figure:dp}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.9\textwidth]{revs.jpg}
    \caption{逆扩散过程}
    \label{figure:revs}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.9\textwidth]{ReconsModel.PNG}
    \caption{图像重建块模型结构}
...
其中，$\mbs{x}_t, t$分别为时刻$t$以及$t$时刻的隐变量。$\mbs{v}$用于引导电导率分布的电压数据。
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{CCDecoder.png}
...
而CEncoder的设计如\cref{table:VAE_Conductivity}所示。
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.45\textwidth]{CLIPLoss.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.45\textwidth]{DDPMLoss.PNG}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.45\textwidth]{DecoderLoss.png}
